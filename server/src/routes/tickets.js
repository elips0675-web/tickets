...
router.get('/', async (req, res) => {
  ...
  try {
    const [[{ total }]] = await knex.raw('SELECT COUNT(*) as total FROM tickets')
...
router.put('/:id/status', requireRole('admin', 'senior_agent'), updateStatusValidation, async (req, res) => {
  ...
  try {
    const [[old]] = await knex.raw('SELECT status FROM tickets WHERE id = ?', [req.params.id])
...
router.put('/:id/priority', requireRole('admin', 'senior_agent'), updatePriorityValidation, async (req, res) => {
  ...
  try {
    const [[old]] = await knex.raw('SELECT priority FROM tickets WHERE id = ?', [req.params.id])
...
router.put('/:id/assign', requireRole('admin', 'senior_agent'), assignTicketValidation, async (req, res) => {
  ...
  try {
    const [[emp]] = employeeId ? await knex.raw('SELECT name FROM employees WHERE id = ?', [employeeId]) : []
...
router.delete('/:id/messages/:msgId', async (req, res) => {
  try {
    const [[msg]] = await knex.raw('SELECT * FROM ticket_messages WHERE id = ?', [req.params.msgId])
    ...
    await knex.raw('DELETE FROM ticket_messages WHERE id = ?', [req.params.msgId])
